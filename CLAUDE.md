...
**[docs/00-이어서-작업하기.md](docs/00-이어서-작업하기.md)** — 지금 어디까지 했고 뭐가 남았는지,
새 컴퓨터에서 시작하는 절차, git에 없어서 복구 불가능한 데이터가 무엇인지.

이 파일(`CLAUDE.md`)은 **코드가 어떻게 생겼는지**를, 저 문서는 **작업이 어디까지 왔는지**를 담는다.
작업을 마칠 때는 저 문서의 "지금 상태 / 최근에 한 작업 / 열려 있는 항목"을 갱신한다.
...
| 경로 | 소속 | 설명 |
|---|---|---|
| `docs/00-이어서-작업하기.md` | 공용 | **작업 진행 상황·인계 문서. 작업을 마칠 때 갱신한다** |
| `apps-script/Code.gs` | index3 전용 | 팀 관리 공유 데이터 백엔드 (605줄) |
